...
void CLineMgr::Update()
{
	...
	ptMouse.x = (int)ptMouse.x;
	ptMouse.y = (int)ptMouse.y;
...
bool CLineMgr::Collision_Line(CObj* pPlayer, float* pY)
{
	...
	float fX = pPlayer->Get_Info().vPos.x;
	float fY = pPlayer->Get_Info().vPos.y;
	...
	for (auto& pLine : m_LineList)
	{
			...
			fX <= pLine->Get_Right().x))
		{
			...
			*pY = ((y2 - y1) / (x2 - x1)) * (fX - x1) + y1;
			...
			{
				float offset = pPlayer->Get_Info().vPos.y * 0.5f + 8.f;
